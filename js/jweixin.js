...
! function(a, b) {
	...
	function e(a) {
		return a = a || {}, a.appId = D.appId, a.verifyAppId = D.appId, a.verifySignType = "sha1", a.verifyTimestamp = D.timestamp + "", a.verifyNonceStr = D.nonceStr, a.verifySignature = D.signature,
			a
	}
	...
	function l(a) {
		if(!w && !D.debug) {
			var b = new Image;
			if(C.appId = D.appId, C.initTime = B.initEndTime - B.initStartTime, C.preVerifyTime = B.preVerifyEndTime - B.preVerifyStartTime, v) {
				var c = "https://open.work.weixin.qq.com/wwopen/report/sdk?v=" + C.version + "&o=" + C.isPreVerifyOk + "&cv=" + C.clientVersion + "&appid=" + C.appId + "&nt=pc&it=" + C.initTime + "&pt=" + C.preVerifyTime + "&url=" + C.url;
				b.src = c;
			} else G.getNetworkType({
				isInnerInvoke: !0,
				success: function(a) {
					C.networkType = a.networkType;
					var c = "https://open.work.weixin.qq.com/wwopen/report/sdk?v=" + C.version + "&o=" + C.isPreVerifyOk + "&cv=" + C.clientVersion + "&appid=" + C.appId + "&nt=" + C.networkType + "&it=" + C.initTime + "&pt=" + C.preVerifyTime + "&url=" + C.url;
					b.src = c
				}
			})
		}
	...
	if(!a.jWeixin) {
		var p = {
			...
			x = -1 != t.indexOf("wxwork"),
			y = -1 != t.indexOf("android"),
			z = -1 != t.indexOf("iphone") || -1 != t.indexOf("ipad"),
			A = function() {
				var a = t.match(/wxwork\/(\d+\.\d+\.\d+)/) || t.match(/wxwork\/(\d+\.\d+)/);
				return a ? a[1] : ""
			}(),
			B = {
				initStartTime: m(),
